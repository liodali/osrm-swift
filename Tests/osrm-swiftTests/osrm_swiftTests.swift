import XCTest
@testable import osrm_swift
import MapKit
import Files
import OHHTTPStubs
import OHHTTPStubsSwift

final class osrm_swiftTests: XCTestCase {
    func testbuildURL() throws {
        let waypoints = [
            CLLocationCoordinate2D(latitude: 52.517037, longitude: 13.388860),
            CLLocationCoordinate2D(latitude: 52.529407, longitude: 13.397634),
            CLLocationCoordinate2D(latitude: 52.523219, longitude: 13.428555),
        ];
        ...
        XCTAssertTrue(serverURL == result)
    }
    func testManagerRoad() async throws{
        do {
            let bundle = Bundle.module

            guard let url = bundle.url(forResource: "example1", withExtension: "json") else {
                XCTFail("Missing file: example1.json")
                return
            }
            let json = try Data(contentsOf: url)
            print(json)
            //let mockedData = try! JSONSerialization.jsonObject(with: response.data(using: .utf8)!,options: [])
            let coords = "13.388860,52.517037;13.397634,52.529407;13.428555,52.523219".toWaypoints()
            let urlMock = URL(string: "https://routing.openstreetmap.de/routed-car/route/v1/driving/13.38886,52.517037;13.397634,52.529407;13.428555,52.523219?steps=true&overview=full&geometries=polyline&alternatives=false")!
          
            // Swift
            let roadJson =  try JSONSerialization.jsonObject(with: json,options: []) as! [String : Any?]
